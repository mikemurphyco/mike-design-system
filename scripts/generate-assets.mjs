...
// ── 7. Hand-authored marks — re-render PNGs from SVGs ────────────────────────
// The Loop and Node SVGs are hand-authored because their geometry is the brand.
// This script still owns their PNG exports so the raster kit stays synced when
// an SVG changes.
...
function renderHandAuthoredPngs({ sourceDir, pngDir, files }) {
  const names = files ?? readdirSync(sourceDir).filter((f) => f.endsWith('.svg'));
  for (const f of names) {
    const svgStr = readFileSync(join(sourceDir, f), 'utf8');
    ...
    const pngDest = join(pngDir, basename(f, '.svg') + '.png');
    renderPngFromSvgFile(svgStr, pngDest, native * 2);
    console.log(`  png  ${pngDest.replace(ROOT + '/', '')}`);
  }
}

console.log('\nLoop mark (re-rendered PNGs from hand-authored SVGs)');
renderHandAuthoredPngs({
  sourceDir: join(ROOT, 'assets/loops'),
  pngDir: join(ROOT, 'assets/loops/png'),
});

console.log('\nNode marks (re-rendered PNGs from hand-authored SVGs)');
for (const nodeDir of ['floating', 'badged', 'legacy']) {
  renderHandAuthoredPngs({
    sourceDir: join(ROOT, 'assets/nodes', nodeDir),
    pngDir: join(ROOT, 'assets/nodes/png'),
  });
}

console.log('\nLoop/Node logo lockups (re-rendered PNGs from hand-authored SVGs)');
renderHandAuthoredPngs({
  sourceDir: join(ROOT, 'assets/logos/lockup'),
  pngDir: join(ROOT, 'assets/logos/lockup/png'),
  files: ['lockup-loop.svg', 'lockup-loop-ai.svg', 'lockup-node.svg'],
});
