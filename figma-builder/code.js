// MM Design System Builder — v2026.2
...
(async () => {
  try {
    figma.notify("Building Mike Murphy DS v2026.1…", { timeout: 2000 });
    ...
    let collection = (await figma.variables.getLocalVariableCollectionsAsync())
      .find((c) => c.name === "Mike Murphy v2026.1");
    if (!collection) {
      collection = figma.variables.createVariableCollection("Mike Murphy v2026.1");
    ...
    // ── 01 COVER ────────────────────────────────────────────
    {
      ...
      const lede = txt("Lede", "Tokens, type, marks, and the component vocabulary for the v2026.1 brand. Consume semantic tokens only — never hardcode hex.", "color/semantic/text-muted", HEX.navy, 0.62);
      ...
      pill.appendChild(txt("Mono Label", "v2026.2", "color/brand/mm-cream", HEX.cream));
    ...
    // ── 10 LOGO & MARKS ─────────────────────────────────────
    {
      ...
      const nodeNavySvg = `<svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" viewBox="0 0 128 128"><circle cx="68" cy="68" r="58" fill="#001E3A" opacity="0.25"/><circle cx="64" cy="64" r="58" fill="#001E3A"/><line x1="64" y1="51" x2="34" y2="28" stroke="#FAF7F2" stroke-width="3" stroke-linecap="round" opacity="0.55"/><line x1="77" y1="58" x2="98" y2="44" stroke="#FAF7F2" stroke-width="2.5" stroke-linecap="round" opacity="0.40"/><line x1="58" y1="77" x2="42" y2="96" stroke="#FAF7F2" stroke-width="2" stroke-linecap="round" opacity="0.28"/><circle cx="64" cy="64" r="13" fill="#FF6434"/><circle cx="30" cy="25" r="9" fill="#FAF7F2"/><circle cx="102" cy="41" r="6.5" fill="#1ECEBE"/><circle cx="39" cy="99" r="4.5" fill="#FAF7F2" opacity="0.35"/></svg>`;
    ...
    // ── 16 FOOTER ──────────────────────────────────────────
    {
      ...
      f.appendChild(txt("Mono Label", "v2026.2", "color/semantic/text-muted", HEX.navy, 0.62));
    }
